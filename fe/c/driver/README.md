...
This crate wires the C frontend crates together behind a tiny, zero-dependency command line
...
`compile_source(name, src, emit, include_dirs)` runs the pipeline in memory and returns the
rendered `output`, a `had_errors` flag, and the collected `diagnostics`, used by the
